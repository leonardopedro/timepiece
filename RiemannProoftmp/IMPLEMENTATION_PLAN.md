...
## Euler Product Approximants

### Partial Euler product and eta counterpart

We define the partial Euler product and its eta counterpart:

    ζ_P(s) = ∏_{p ≤ P, p prime} 1/(1 − p^{−s})
    η_P(s) = (1 − 2^{1−s}) · ζ_P(s)

### Non-vanishing

For Re(s) > 1/2, each factor (1 − p^{−s}) is non-zero since
|p^{−s}| = p^{−Re(s)} < 1. Therefore ζ_P(s) ≠ 0 and η_P(s) ≠ 0
whenever Re(s) < 1 (where the eta factor 1 − 2^{1−s} is also
non-zero).

### Analyticity

Each η_P(s) is a finite product of analytic functions, hence analytic
on {Re(s) > 0}. Its reciprocal 1/η_P(s) is analytic on any rectangle
R ⊂ {1/2 < Re < 1}.

These approximants provide the non-vanishing analytic building blocks
that, through Bagchi universality, allow us to construct the correcting
factors h_n.

---

## Bagchi Correction of the Finite Euler Products

### The exponential connection

For Re(s) > 1/2, the relationship between the finite product ζ_P(s)
and ζ(s) is:

    ζ_P(s) = ζ(s) · exp(−T_P(s) − H_P(s))

where the correction exponent decomposes into:

- **Prime Zeta Tail**: T_P(s) = Σ_{p>P} p^{−s}, which is only
  conditionally convergent for Re(s) > 1/2.
- **Higher Prime Sum**: H_P(s) = Σ_{p>P} Σ_{k≥2} p^{−ks}/k, which
  is absolutely convergent for Re(s) > 1/2 since |p^{−ks}/k| ≤ p^{−2σ}/2.

The finite product ζ_P(s) deviates from ζ(s) by the multiplicative
factor exp(−T_P(s) − H_P(s)). For ζ_P → ζ uniformly, we need this
correction factor to converge uniformly to 1.

### Bagchi uniform correction

Bagchi's universality theorem guarantees the Euler product tail can be
controlled uniformly on compact subsets of {1/2 < Re < 1}:

- **Prime zeta tail** (axiom `primeZetaTail_uniform_small`): For any
  compact K ⊂ {Re(s) ≥ 1/2 + α} (α > 0), T_P(s) is uniformly small.
  By PNT + Abel summation: |T_P(s)| = O(P^{1/2−σ}/log P) → 0.
  Bagchi guarantees no resonances from conditional convergence.

- **Higher prime sum** (axiom `higherPrimeSum_uniform_small`):
  H_P(s) is uniformly small by absolute convergence:
  Σ_p p^{−2σ} → 0 as P → ∞ for σ > 1/2.

### From correction to uniform convergence

Let w_P(s) = −T_P(s) − H_P(s). For P large, ‖w_P(s)‖ < 1/2 uniformly.
Using ‖e^w − 1‖ ≤ 2‖w‖ (Mathlib: `Complex.norm_exp_sub_one_le`):

    ‖ζ_P(s) − ζ(s)‖ ≤ ‖ζ(s)‖ · 2‖w_P(s)‖ < ‖ζ(s)‖ · 2(‖T_P‖ + ‖H_P‖)

With ‖ζ(s)‖ bounded by M on compact K, and both tails < ε/(4M):

    ‖ζ_P(s) − ζ(s)‖ < ε  uniformly on K

Multiplying by the bounded eta factor preserves uniform convergence:

    ‖η_P(s) − η(s)‖ ≤ B · ε  where B = sup_K |1 − 2^{1−s}|

Therefore η_P → η uniformly on compact subsets of {1/2 < Re < 1}.

**This uniform convergence is what underpins the construction of h_n**:
since η_P → η uniformly and η_P ≠ 0, Bagchi universality allows
approximating any target function (such as s − 3/4 − i) on contours
where 1/η has only simple poles.
...
### Task 0: Formalize Euler product approximants and Bagchi correction

**Goal**: Define ζ_P(s) and η_P(s), prove their non-vanishing and
analyticity, and formalize the Bagchi correction framework:
- `zetaEulerProd P s = ∏_{p ≤ P} (1 - p^{-s})⁻¹`
- `etaEulerApprox P s = (1 - 2^{1-s}) * zetaEulerProd P s`
- Non-vanishing: `etaEulerApprox P s ≠ 0` for Re(s) ∈ (1/2, 1)
- Analyticity: `etaEulerApprox_analyticOnNhd`
- Exponential connection: `eulerProd_zeta_exp_connection` axiom
- Uniform convergence: `etaEulerApprox_tendstoUniformlyOn_rect`

**Status**: Mostly implemented in `RectangleStrategy.lean` (already
proved: `etaEulerApprox_ne_zero`, `etaEulerApprox_analyticOnNhd`,
`etaEulerApprox_tendstoUniformlyOn_rect`). The three axioms
(`primeZetaTail_uniform_small`, `higherPrimeSum_uniform_small`,
`eulerProd_zeta_exp_connection`) encode the Bagchi correction.

**Adaptation for η(2s−1)**: The existing lemmas operate on η(s);
for the shifted function η(2s−1), compose with the map s ↦ 2s−1.
...
```
Task 0 (Euler product approximants + Bagchi correction)
  │  existing: etaEulerApprox, primeZetaTail, etc.
  │  adapt to shifted argument 2s−1
  │
Task 1 (define etaShifted + approximants f_n = h_n · η_n)
  │  uses: Task 0 infrastructure
  │
Task 2 (h_n → (s−3/4−i) uniformly on ∂R via Bagchi)
  │  uses: Task 0 uniform convergence, Bagchi axioms
  │
Task 2b (η_n → η(2s−1) conditionally)
  │  uses: EtaConvergence.lean paired-term summation
  │
...
1. **Task 0** — Euler product approximants and Bagchi correction.
   Mostly already in RectangleStrategy.lean; adapt to 2s−1 argument.

2. **Tasks 1–3** — Definitions and convergence. Adapt existing
   infrastructure to the shifted argument, define h_n via Bagchi on ∂R.

3. **Tasks 4–6** — Contour integral machinery. Rectangle decomposition,
   edge bounds via uniform convergence, real-axis positivity.

4. **Task 7** — The multiplicity-one contradiction. Core insight:
   residue sign vs. real-axis positivity, even-multiplicity exclusion.

5. **Tasks 8–9** — Zero location via four-fold product. Structurally
   similar to Task 7 with functional equation symmetry.
