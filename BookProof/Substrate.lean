...
formalism is already formalized in `BookProof/PhysMehler.lean` (substrate bundle,
`BookProof/PhysHSGaussian.lean` Mehler chain,
`BookProof/PhysFunctionalAnalysis.lean` L²/√p layer).  Here we
...
theorem substrate_born_forward (p : ℝ → ℝ) (hmeas : Measurable p)
    (hnonneg : ∀ x, 0 ≤ p x) (hint : ∫ x, p x ∂PhysMeasureBasis.unitMeasure = 1) :
    ∃ Ψ : Lp ℂ 2 PhysMeasureBasis.unitMeasure,
      ‖Ψ‖ = 1 ∧ ∀ᵐ x ∂PhysMeasureBasis.unitMeasure, ‖(Ψ : ℝ → ℂ) x‖ ^ 2 = p x :=
  BookProof.ChapterB.born_forward PhysMeasureBasis.unitMeasure p hmeas hnonneg hint

/-- **Chapter B.1 backward at the substrate.** Conversely, `|Ψ|²` of a unit
wave-function over the substrate is a probability density. -/
theorem substrate_born_backward (Ψ : Lp ℂ 2 PhysMeasureBasis.unitMeasure) (hΨ : ‖Ψ‖ = 1) :
    (∀ x, 0 ≤ ‖(Ψ : ℝ → ℂ) x‖ ^ 2) ∧
      ∫ x, ‖(Ψ : ℝ → ℂ) x‖ ^ 2 ∂PhysMeasureBasis.unitMeasure = 1 :=
  BookProof.ChapterB.born_backward PhysMeasureBasis.unitMeasure Ψ hΨ

/-- **Chapter B.2 at the substrate.** Every unit wave-function over the substrate
is a member of a Hilbert basis — the book's `Ψ = 𝒰 e₀`. -/
theorem substrate_unit_vector_extends (Ψ : Lp ℂ 2 PhysMeasureBasis.unitMeasure) (hΨ : ‖Ψ‖ = 1) :
    ∃ (ι : Type) (b : HilbertBasis ι ℂ (Lp ℂ 2 PhysMeasureBasis.unitMeasure)) (i : ι), b i = Ψ :=
