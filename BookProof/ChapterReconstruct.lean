  ...
  have h_C_zero : C = 0 := by
    ...
    have hz : ∀ z : ℂ, (starRingEnd ℂ) z * C + z * (starRingEnd ℂ) C = 0 := by
      ...
      have hz : offDiag U a (fun k => if k = m then 1 else if k = l then z else 0) = (starRingEnd ℂ) z * C + z * (starRingEnd ℂ) C := by
        rw [ offDiag_eq ];
        simp +decide [ Finset.sum_ite, Finset.filter_eq', Finset.filter_ne', hlm ] ; ring;
  ...
  · refine' fun a => isDeterministicCol_of_offDiag U a _;
    ...
    · -- Let $c = \sqrt{\sum_{k} \| \Psi_k \|^2}$.
      ...
      have hc_unit : ∑ k, ‖(1 / c : ℂ) * Ψ k‖ ^ 2 = 1 := by
        simp +decide [ mul_pow, ← Finset.mul_sum _ _ _ ];
        rw [ Real.sq_sqrt <| Finset.sum_nonneg fun _ _ => sq_nonneg _, inv_mul_cancel₀ <| ne_of_gt <| lt_of_le_of_ne ( Finset.sum_nonneg fun _ _ => sq_nonneg _ ) <| Ne.symm <| by contrapose! hΨ; ext i; simp_all +decide  ]
