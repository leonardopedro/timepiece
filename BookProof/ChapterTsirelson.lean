...
theorem chshOp_eq_tuple :
  ...
  congr <;> ext i j;
  · fin_cases i <;> fin_cases j <;> simp only [Fin.zero_eta, Fin.isValue, Matrix.kroneckerMap_apply, Matrix.mul_apply, Fin.mk_one];
    all_goals erw [ Finset.sum_product ] ; simp [ Matrix.one_apply ] ;
  · fin_cases i <;> fin_cases j <;> simp only [Fin.zero_eta, Fin.isValue, Matrix.kroneckerMap_apply, Matrix.mul_apply, Fin.mk_one];
    all_goals erw [ Finset.sum_product ] ; simp [ Matrix.one_apply ] ;
  · fin_cases i <;> fin_cases j <;> simp only [Fin.zero_eta, Fin.isValue, Matrix.kroneckerMap_apply, Matrix.mul_apply, Fin.mk_one];
    all_goals simp only [Fin.isValue, Matrix.one_apply, mul_ite, mul_one, mul_zero, ite_mul, one_mul, zero_mul] ;
    all_goals erw [ Finset.sum_product ] ; simp  ;
  · simp only [A1, B1, one_div, Matrix.kroneckerMap_apply, Matrix.smul_apply, Matrix.sub_apply, smul_eq_mul, Matrix.mul_apply];
    fin_cases i <;> fin_cases j <;> simp only [Fin.zero_eta, Fin.isValue, Matrix.one_apply, mul_ite, mul_one, mul_zero, ite_mul, one_mul, zero_mul, Fin.mk_one];
    all_goals erw [ Finset.sum_product ] ; simp  ;
...
theorem chshOp_eigenvector :
  ...
  ext ⟨ i, j ⟩;
  fin_cases i <;> fin_cases j <;> simp only [chshOp, A0, sz, B0, one_div, sx, of_add_of, add_cons, head_cons, add_zero, tail_cons, zero_add, empty_add_empty, smul_of, smul_cons, smul_eq_mul, mul_one, smul_empty, mul_neg, B1, of_sub_of, sub_cons, sub_zero, zero_sub, sub_self, zero_empty, A1, Fin.zero_eta, Fin.isValue, Complex.ofReal_mul, Complex.ofReal_ofNat, Pi.smul_apply, bellState, ↓reduceIte, ne_eq, Complex.ofReal_eq_zero, Nat.ofNat_nonneg, Real.sqrt_eq_zero, OfNat.ofNat_ne_zero, not_false_eq_true, mul_inv_cancel_right₀, Fin.mk_one, Prod.mk.injEq, one_ne_zero, and_false, zero_ne_one, and_true, mul_zero, and_self];
  · simp only [mulVec, dotProduct, kroneckerMap, Matrix.of_apply, cons_val', cons_val_fin_one, of_add_of, Fin.isValue, Matrix.sub_apply, Pi.add_apply, Matrix.cons_val_zero, Fintype.sum_prod_type, Fin.sum_univ_two, add_sub_cancel_right, Matrix.cons_val_one, mul_neg, add_neg_cancel, zero_add, sub_neg_eq_add, one_mul, zero_mul, add_zero]
    unfold bellState; norm_num; ring; norm_num [← Complex.ofReal_pow]
  · simp only [mulVec, dotProduct, kroneckerMap, Matrix.of_apply, cons_val', cons_val_fin_one, of_add_of, Fin.isValue, Matrix.sub_apply, Pi.add_apply, Matrix.cons_val_zero, Matrix.cons_val_one]
    erw [Finset.sum_product]; norm_num [Fin.sum_univ_succ, bellState]
  · simp only [mulVec, dotProduct, kroneckerMap, Matrix.of_apply, cons_val', cons_val_fin_one, of_add_of, Fin.isValue, Matrix.sub_apply, Pi.add_apply, Matrix.cons_val_one, Matrix.cons_val_zero, Fintype.sum_prod_type, Fin.sum_univ_two, add_sub_cancel_right, mul_neg, add_neg_cancel, zero_add, sub_neg_eq_add, zero_mul, add_zero, one_mul, neg_mul]
