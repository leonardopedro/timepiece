...
-- (they are `local` in `BookProof.PhysMehler` and not re-exported).  They are
-- exported globally here because the downstream Solovay chapter needs them.
...
theorem outer_inner_reduces_to_head {N : ℕ} {headDist : Measure (InnerHead N)}
  ...
  have h_tail_ne_zero : tailMeasure ≠ 0 := by
    ...
    intro h_eq
    have h_univ_zero : tailMeasure Set.univ = 0 := by simp [h_eq]
  ...
  have h_mem_g₁ : MemLp g₁' 2 headDist := by
    have h_ae_map : AEStronglyMeasurable g₁'
        (Measure.map Prod.fst (headDist.prod tailMeasure)) := by
  ...
  let g₂ : Lp ℂ 2 headDist := h_mem_g₁.toLp g₁'
  have h_inner_eq : inner ℂ Ψ₁ Ψ₂ = ∫ z : InnerSpace N,
  ...
  have h_fubini_eq : ∫ z : InnerSpace N, (g₂' z.1) * star (g₁' z.1) ∂(headDist.prod tailMeasure) =
    ...
    have h_fubini : ∫ z : InnerSpace N, (g₂' z.1) * star (g₁' z.1) ∂(headDist.prod tailMeasure) =
      ...
      exact integral_prod_symm
        (fun z : InnerHead N × InnerTail => (g₂' z.1) * star (g₁' z.1)) h_int_comp
    rw [h_fubini]
    simp [integral_const]
...
instance {N : ℕ} (x : InnerHead N) (ε : ℝ) [Fact (0 < ε)] :
    IsProbabilityMeasure (normalizedBumpMeasure x ε) := by
  have hε_pos : 0 < ε := Fact.out
  have h2ε_nonneg : 0 ≤ 2 * ε := by linarith
  have h_comp (i : Fin N) :
      (volume.restrict (Set.Icc (x i - ε) (x i + ε))) Set.univ = ENNReal.ofReal (2 * ε) := by
...
theorem E_smul {N : ℕ} (headDist : Measure (InnerHead N)) [IsProbabilityMeasure headDist]
    (c : ℂ) (f : InnerHead N → ℂ) (_hf : Integrable f headDist) :
...
theorem Var_orthogonal_sum {N : ℕ} (headDist : Measure (InnerHead N))
    ...
    (_h_mean_g : ∫ x : InnerHead N, g x ∂headDist = 0) :
  ...
  have h_cross : ∫ x : InnerHead N, (f x * star (g x)).re ∂headDist = 0 := by
    ...
    have h_indep_f_starg : IndepFun f (fun x => star (g x)) headDist :=
      IndepFun.comp (φ := id) (ψ := star) h_indep measurable_id continuous_star.measurable
  ...
  calc
    ∫ x : InnerHead N, Complex.normSq (f x + g x) ∂headDist =
      ...
      refine integral_congr_ae (ae_of_all _ (fun x => ?_))
      simp [Complex.normSq_add, Complex.mul_re, Complex.conj_re]
    _ = (∫ x : InnerHead N, Complex.normSq (f x) ∂headDist) +
      ...
      have h_int_cross_val : ∫ x : InnerHead N, 2 * ((f x * star (g x)).re) ∂headDist =
          2 * (∫ x : InnerHead N, (f x * star (g x)).re ∂headDist) := by
        rw [MeasureTheory.integral_const_mul (2 : ℝ)
          (fun x => (f x * star (g x)).re) (μ := headDist)]
...
theorem Var_smul {N : ℕ} (headDist : Measure (InnerHead N)) [IsProbabilityMeasure headDist]
    (c : ℂ) (f : InnerHead N → ℂ) (_hf : MemLp f 2 headDist) :
    ...
    calc
      (∫ x in (-Real.sqrt ε)..(Real.sqrt ε), x ^ 2) =
        ...
        apply (intervalIntegral.integral_add_adjacent_intervals
          ((continuous_id.pow 2).intervalIntegrable _ _)
          ((continuous_id.pow 2).intervalIntegrable _ _)).symm
      _ = (∫ x in (0 : ℝ)..(Real.sqrt ε), (-x) ^ 2) + (∫ x in (0 : ℝ)..(Real.sqrt ε), x ^ 2) := by
        have h_sq_eq : (∫ x in (-Real.sqrt ε)..(0 : ℝ), x ^ 2) =
            (∫ x in (-Real.sqrt ε)..(0 : ℝ), (-x) ^ 2) := by
          refine intervalIntegral.integral_congr_ae (ae_of_all volume (fun x _ => ?_))
          ring
        have h := intervalIntegral.integral_comp_neg
          (fun x : ℝ => x ^ 2) (a := -Real.sqrt ε) (b := 0)
        have h_simplified : (∫ x in (-Real.sqrt ε)..(0 : ℝ), (-x) ^ 2) =
            (∫ x in (0 : ℝ)..(Real.sqrt ε), x ^ 2) := by
          simpa using h
        have h_C_eq_B : (∫ x in (0 : ℝ)..(Real.sqrt ε), (-x) ^ 2) =
            (∫ x in (0 : ℝ)..(Real.sqrt ε), x ^ 2) := by
          refine intervalIntegral.integral_congr_ae (ae_of_all volume (fun x _ => ?_))
          ring
...
/-- The exact second moment of the random walk on `omegaMeasure`:
    ...
    the infinite zeta function. -/
theorem uniform_variance_bound {N : ℕ} (ε : ℝ) (hε : 0 < ε) (n : ℕ) (_hn : n ≥ 1) :
    ∫ x : Fin (N+1) → ℝ, (∑ i : Fin (N+1), (x i) ^ 2) ∂(omegaMeasure ε) =
    (N+1 : ℝ) * (1 + ε/3) * ((2 * Real.sqrt ε) ^ (N+1)) := by
  have h_sqrt_pos : 0 < Real.sqrt ε := Real.sqrt_pos.mpr hε
  have h_int (i : Fin (N+1)) :
      Integrable (fun x : Fin (N+1) → ℝ => (x i) ^ 2) (omegaMeasure ε) := by
    dsimp [omegaMeasure]
  ...
  have h_coord (i : Fin (N+1)) : ∫ x : Fin (N+1) → ℝ, (x i) ^ 2 ∂(omegaMeasure ε) =
    ...
    have h_prod_mass : (∏ j ∈ Finset.univ.erase i,
      ...
      simp_rw [MeasureTheory.Measure.restrict_apply_univ
        (s := Set.Icc (1 - Real.sqrt ε) (1 + Real.sqrt ε)), Real.volume_Icc]
  ...
  calc
    ...
    _ = (Finset.card (Finset.univ : Finset (Fin (N+1))) : ℝ) *
        ((1 + ε/3) * ((2 * Real.sqrt ε) ^ (N+1))) := by
...
theorem moore_osgood_commutation {N : ℕ} (ε : ℝ) (hε : 0 < ε) :
  ...
  have h_sqrt_pos : 0 < Real.sqrt ε := Real.sqrt_pos.mpr hε
  have h_int (i : Fin (N+1)) :
      Integrable (fun x : Fin (N+1) → ℝ => (x i - 1) ^ 2) (omegaMeasure ε) := by
    dsimp [omegaMeasure]
...
theorem jensen_bohr {N : ℕ} (headDist : Measure (InnerHead N)) [IsProbabilityMeasure headDist]
  ...
  have h_sum : Summable fun n : ℕ => (ArithmeticFunction.moebius n : ℂ) / (n : ℂ) ^ s := by
    ...
    have h_term_eq : (fun n : ℕ => LSeries.term
        (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) s n) =
...
theorem convergent_series_has_no_poles {N : ℕ} (headDist : Measure (InnerHead N))
    ...
    (s : ℂ) (hs : s.re > s₀.re) :
    DifferentiableAt ℂ
      (fun s' : ℂ => ∑' n : ℕ, (ArithmeticFunction.moebius n : ℂ) / (n : ℂ) ^ s') s := by
  have h_moebius_zero : (ArithmeticFunction.moebius 0 : ℂ) = 0 := by simp
  have h_lseries : LSeriesSummable (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) s₀ := by
    rw [LSeriesSummable]
    have h_term_eq : LSeries.term
        (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) s₀ =
        (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ) / (n : ℂ) ^ s₀) := by
      ext n
      by_cases hn : n = 0
      · simp [hn]
    ...
    exact h_conv
  have h_abscissa :
      LSeries.abscissaOfAbsConv (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) ≤ s₀.re :=
    LSeriesSummable.abscissaOfAbsConv_le h_lseries
  have h_lt :
      LSeries.abscissaOfAbsConv (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) < s.re :=
    lt_of_le_of_lt h_abscissa (by exact_mod_cast hs)
  have h_diff_on : DifferentiableOn ℂ
      (LSeries (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)))
  ...
  have h_mem : s ∈ {s' | LSeries.abscissaOfAbsConv
      (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) < s'.re} := h_lt
  have h_diff_at : DifferentiableAt ℂ
      (LSeries (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ))) s :=
    h_diff_on.differentiableAt (by
      have h_open : IsOpen
          {s' : ℂ | LSeries.abscissaOfAbsConv (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ))
            < (s'.re : EReal)} := by
        ...
        have h_open_Ioi : IsOpen
            {x : EReal | LSeries.abscissaOfAbsConv
              (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) < x} :=
          isOpen_Ioi
        -- The preimage of an open set under a continuous map is open
        exact h_cont.isOpen_preimage _ h_open_Ioi
      rw [mem_nhds_iff]
      exact ⟨{s' | LSeries.abscissaOfAbsConv
          (fun n : ℕ => (ArithmeticFunction.moebius n : ℂ)) < (s'.re : EReal)},
...
theorem L2_cylindrical_norm_diff {X Y : Type*}
  ...
  have h_mem_comp : MemLp (fun z : X × Y => (f - g) z.1) 2 (μ.prod ν) := by
    ...
    have h_equiv :=
      MeasureTheory.memLp_map_measure_iff (p := 2) h_ae_map measurable_fst.aemeasurable
  ...
  calc
    ∫ z, ‖(f - g) z.1‖^2 ∂(μ.prod ν) = ∫ y, ∫ x, ‖(f - g) x‖^2 ∂μ ∂ν := by
      simpa using integral_prod_symm (fun z : X × Y => ‖(f - g) z.1‖^2) h_int
...
theorem cross_factor_expectation {X Y : Type*}
    [MeasurableSpace X] [MeasurableSpace Y]
    (μ : Measure X) (ν : Measure Y) [IsProbabilityMeasure μ] [IsProbabilityMeasure ν]
    (f : X → ℂ) (_hf : Integrable f μ)
    (g : Y → ℂ) (_hg : Integrable g ν) :
...
theorem Var_sq_coord {N : ℕ} (x : InnerHead N) (ε : ℝ) [Fact (0 < ε)] (i : Fin N) :
  ...
  have h_1d_one (a : ℝ) : ∫ y in Set.Icc (a - ε) (a + ε), (1 : ℝ) = 2 * ε := by
    rw [MeasureTheory.integral_Icc_eq_integral_Ioc,
      ← intervalIntegral.integral_of_le (by linarith : a - ε ≤ a + ε)]
    simp [intervalIntegral.integral_const,
      show (a + ε) - (a - ε) = 2 * ε by ring]
  ...
  have h_second_bump : ∫ y : InnerHead N, (y i - x i)^2 ∂(bumpMeasure x ε) =
    ...
    have h_factor : (fun y : InnerHead N => (y i - x i)^2) =
        (fun y => ∏ j : Fin N,
          (fun t : ℝ => if j = i then (t - x i)^2 else 1) (y j)) := by
      ext y; simp
    rw [h_factor]
    rw [integral_fintype_prod_eq_prod (fun j t => if j = i then (t - x i)^2 else 1)]
    have h_i : (∫ y in Set.Icc (x i - ε) (x i + ε), (y - x i)^2) = (2 * ε^3) / 3 :=
      h_1d_second (x i)
    have h_others (j : Fin N) (hj : j ≠ i) :
        (∫ y in Set.Icc (x j - ε) (x j + ε), (1 : ℝ)) = 2 * ε :=
      h_1d_one (x j)
    have h_prod : (∏ j : Fin N,
        (∫ y in Set.Icc (x j - ε) (x j + ε), (if j = i then (y - x i)^2 else 1))) =
  ...
  have h_fourth_bump : ∫ y : InnerHead N, (y i - x i)^4 ∂(bumpMeasure x ε) =
    ...
    have h_others (j : Fin N) (hj : j ≠ i) :
      ...
      h_1d_one (x j)
    have h_prod : (∏ j : Fin N,
        (∫ y in Set.Icc (x j - ε) (x j + ε),
          (if j = i then (y - x i)^4 else 1))) =
  ...
  · have hNpos : 1 ≤ N := by omega
    ...
    have h_pow_div (h2ε_ne : 2 * ε ≠ 0) :
        (((2 * ε) ^ (N-1 : ℕ) : ℝ) / ((2 * ε) ^ N : ℝ)) = 1 / (2 * ε) := by
...
theorem Covariance_bound {N : ℕ} (headDist : Measure (InnerHead N)) [IsProbabilityMeasure headDist]
    (f g : InnerHead N → ℂ) (hf : MemLp f 2 headDist) (hg : MemLp g 2 headDist)
    (_h_indep : IndepFun f g headDist)
    (_h_mean_f : ∫ x : InnerHead N, f x ∂headDist = 0)
    (_h_mean_g : ∫ x : InnerHead N, g x ∂headDist = 0) :
  ...
  have h_int : Integrable
      (fun x => (f x * star (g x)).re) headDist := by
...
theorem Correlation_bound {N : ℕ} (headDist : Measure (InnerHead N)) [IsProbabilityMeasure headDist]
    (f g : InnerHead N → ℂ) (hf : MemLp f 2 headDist) (hg : MemLp g 2 headDist)
    (_h_var_f_pos : 0 < ∫ x : InnerHead N, ‖f x‖ ^ 2 ∂headDist)
    (_h_var_g_pos : 0 < ∫ x : InnerHead N, ‖g x‖ ^ 2 ∂headDist) :
    |∫ x : InnerHead N, (f x * star (g x)).re ∂headDist| ≤
    Real.sqrt ((∫ x : InnerHead N, ‖f x‖ ^ 2 ∂headDist) *
              (∫ x : InnerHead N, ‖g x‖ ^ 2 ∂headDist)) := by
