#!/usr/bin/env python3
"""
Upload timepiece theorem STAMENTS to Prove2me.
One-step: POST /submit-problem per theorem.
Fast - no compilation polling.

Usage:
    python3 upload_timepiece_v3.py --dry-run    # preview
    python3 upload_timepiece_v3.py --limit 10    # test
    python3 upload_timepiece_v3.py             # full
"""
...
STATE_FILE = Path("/tmp/timepiece_upload_v3_state.json")
LOG_FILE = Path("/tmp/timepiece_upload_v3.log")
...
for i in range(start_idx, len(new_theorems)):
    ...
    if "jobs" in resp and resp["jobs"]:
        ...
        with open(LOG_FILE, "a") as f:
            f.write(f"OK: {prefix}.{name} (job: {job_id}){tag}\n")
        ok += 1
    else:
        errors = resp.get("errors", [])
        with open(LOG_FILE, "a") as f:
            f.write(f"FAIL: {prefix}.{name} - {errors}{tag}\n")
        fail += 1
