...
def liInt (D J n : ℕ) : ℕ :=
  (sumRange (n + 1) (fun k =>
    if k < 2 then 0 else (D * D) / lnFix D J k)) / D
...
theorem pow_primrec : Primrec₂ (fun a b : ℕ => a ^ b) := by
  have h_primrec : Primrec (fun p : (ℕ × ℕ) => p.1 ^ p.2) := by
    ...
    · exact fun p => p.2
    · exact fun p => 1
    · exact fun p (n IH) => p.1 * IH
    · exact funext fun p => by
        induction p.2 <;> simp +decide [*, pow_succ']
    · exact Primrec.snd
    · exact Primrec.const 1
    · exact Primrec.nat_mul.comp (Primrec.fst.comp Primrec.fst)
...
theorem prime_primrec : PrimrecPred Nat.Prime := by
  have h_div_rec : PrimrecRel (fun (m p : ℕ) => m ∣ p → m = 1) := by
    have h_mod : Primrec (fun p : ℕ × ℕ => p.2 % p.1) :=
      Primrec.nat_mod.comp Primrec.snd Primrec.fst
    have h_mod' : PrimrecPred (fun p : ℕ × ℕ =>
        p.2 % p.1 = 0 → p.1 = 1) := by
      convert Primrec.eq.comp h_mod (Primrec.const 0)
        |> PrimrecPred.not
        |> PrimrecPred.or <| Primrec.eq.comp Primrec.fst (Primrec.const 1) using 1
      grind
  ...
  · constructor <;> intro h
    · convert PrimrecRel.forall_lt h_div_rec using 1
    · convert PrimrecPred.and
        (Primrec.nat_le.comp (Primrec.const 2) Primrec.id)
        (h.comp Primrec.id Primrec.id) using 1
...
theorem sumRange_primrec {α : Type*} [Primcodable α] {m : α → ℕ} {g : α → ℕ → ℕ}
  ...
  have h_fold : Primrec (fun a =>
      List.foldr (fun k acc => g a k + acc) 0 (List.range (m a))) := by
    have h_g : Primrec (fun (p : α × ℕ) => g p.1 p.2) := hg
    have h_range : Primrec (fun (a : α) => List.range (m a)) :=
      Primrec.list_range.comp hm
    convert Primrec.list_foldr h_range (Primrec.const 0)
      (show Primrec₂ (fun a => fun p : ℕ × ℕ =>
        g a p.1 + p.2) from ?_) using 1
    exact Primrec.nat_add.comp
      (h_g.comp (Primrec.fst.pair Primrec.fst |>.comp Primrec.snd))
      (Primrec.snd.comp Primrec.snd)
  exact h_fold

theorem primePi_primrec : Primrec primePi := by
  have h_length_primrec : Primrec (fun l : List ℕ => l.length) :=
    Primrec.list_length
  have h_filter_primrec : Primrec (fun l : List ℕ =>
      List.filter (fun k => decide (Nat.Prime k)) l) := by
    apply Primrec.listFilter; exact prime_primrec
  convert h_length_primrec.comp (h_filter_primrec.comp
    (Primrec.list_range.comp Primrec.succ)) using 1

theorem incr_primrec : Primrec (fun p : ℕ × ℕ × ℕ => incr p.1 p.2.1 p.2.2) := by
  have h_g_primrec : Primrec₂
      (fun (p : ℕ × ℕ × ℕ) (j : ℕ) =>
        p.1 / ((2 * j + 1) * (2 * p.2.2 + 1) ^ (2 * j + 1))) := by
    refine Primrec.nat_div.comp (Primrec.fst.comp ?_) _
    · exact Primrec.fst
    refine Primrec.nat_mul.comp ?_ ?_
    · exact Primrec.nat_add.comp
        (Primrec.nat_mul.comp (Primrec.const 2) Primrec.snd) (Primrec.const 1)
    · convert pow_primrec.comp
...
theorem lnFix_primrec : Primrec (fun p : ℕ × ℕ × ℕ => lnFix p.1 p.2.1 p.2.2) := by
  ...
  · exact fun p q =>
      q.2 + (if q.1 = 0 then 0 else incr p.1 p.2.1 q.1)
  · aesop
  · convert Primrec.nat_add.comp (Primrec.snd.comp Primrec.snd)
      (Primrec.ite _ (Primrec.const 0)
        (incr_primrec.comp
          (Primrec.fst.comp Primrec.fst |>.pair
            (Primrec.fst.comp Primrec.snd |>.comp Primrec.fst) |>.pair
            (Primrec.fst.comp Primrec.snd)))) using 1
...
theorem schoenfeld_primrec : Primrec schoenfeld := by
  have h_schoenfeld_comp :
      Primrec (fun n => 6316547 * (absDiff (primePi n)
        (liInt ((n + 1) ^ 2) n n)) ^ 2 * ((n + 1) ^ 2) ^ 2) ∧
      Primrec (fun n => 10000 * n * (lnFix ((n + 1) ^ 2) n n) ^ 2) := by
    constructor
    · apply Primrec.nat_mul.comp (Primrec.nat_mul.comp (Primrec.const 6316547)
        (pow_primrec.comp
          (Primrec.nat_add.comp
            (Primrec.nat_sub.comp primePi_primrec
              (liInt_primrec.comp
                (Primrec.pair
                  (pow_primrec.comp Primrec.succ (Primrec.const 2))
                  (Primrec.pair Primrec.id Primrec.id))))
        ...
        (pow_primrec.comp
          (pow_primrec.comp Primrec.succ (Primrec.const 2))
          (Primrec.const 2))
    · have h_schoenfeld_primrec : Primrec
          (fun n => lnFix ((n + 1) ^ 2) n n) := by
        convert lnFix_primrec.comp
          (Primrec.pair
            (Primrec.comp (pow_primrec.comp Primrec.succ (Primrec.const 2)) Primrec.id)
            (Primrec.pair Primrec.id Primrec.id)) using 1
      convert Primrec.nat_mul.comp
        (Primrec.nat_mul.comp (Primrec.const 10000) Primrec.id)
        (h_schoenfeld_primrec.comp Primrec.id
          |> Primrec.comp (pow_primrec.comp Primrec.id (Primrec.const 2))) using 1
