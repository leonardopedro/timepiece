...
unsafe def main (args : List String) : IO UInt32 := do
  ...
  -- Parse trailing `-Dname=value` script args into Lean options (the lakefile's
  -- `[leanOptions]` are normally baked into `--setup` by `lake build`, but this
  -- script re-elaborates the file itself, so the runner passes them explicitly).
  let opts ← (args.drop 2).foldlM (fun o a =>
    match a.splitOn "=" with
    | [k, v] =>
      let n := k.dropWhile (· == '-')
      if n.startsWith "D" then
        let name : Name := (n.drop 1).toName
        try
          let decl ← Lean.getOptionDecl name
          let dv : Lean.DataValue :=
            match decl.defValue with
            | Lean.DataValue.ofBool _ => Lean.DataValue.ofBool (v == "true")
            | Lean.DataValue.ofNat _ => Lean.DataValue.ofNat (v.toNat?.getD 0)
            | _ => Lean.DataValue.ofString v
          pure (o.insert name dv)
        catch _ => pure o
      else pure o
    | _ => pure o) Lean.Options.empty
