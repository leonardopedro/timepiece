...
> **Wave note (2026-09-22d, the Standard-Model wave).**  The four new chapters
> `BookProof.ChapterSmOneParticle`, `BookProof.ChapterSmHamiltonian`,
> `BookProof.ChapterSmComparison` and `BookProof.ChapterSmOuterFock` join the operator-theoretic
> core: the last two are the maximal modules of the group and are listed as roots of
> `BookProofOperatorCore` in `lakefile.toml`, the first two being imported by them.  The audit is
> `Work.SmStandardModelAudit`.  The module and root counts of the table below predate this wave
> and are regenerated by `python3 scripts/import_components.py BookProof --markdown`.

### `BookProof` — 918 modules, 112 independent parts (27 with more than one module)

| modules | Lake target | maximal modules (the `roots` of the target) |
| ---: | --- | --- |
| 522 | `BookProofOperatorCore` | `BookProof.ChapterAbelianCyclicCommutant`, `BookProof.ChapterAbelianMixture`, `BookProof.ChapterBddBelowFiberSumEsa`, `BookProof.ChapterBornMeasure`, `BookProof.ChapterBrstUnboundedLeakage`, `BookProof.ChapterCarlemanGeneralHop`, … (78 in total) |
...
| 36 | `BookProofMeasureFoundations` | `BookProof.ChapterB7`, `BookProof.ChapterF2`, `BookProof.ChapterF4`, `BookProof.ChapterF8`, `BookProof.ChapterG3`, `BookProof.ChapterGaugeCasimirAverage`, … (15 in total) |
...
The remaining 85 parts are single modules, each built by `lake build <module>`:
